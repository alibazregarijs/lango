...
const Page = () => {
  // Sample chat data (static for UI demonstration)
  const messages = [
    { id: 1, text: "Hey there! How's your project going?", sender: "user", time: "10:30 AM", seen: true },
    { id: 2, text: "It's going well! Just finished the UI design. What about you?", sender: "other", time: "10:32 AM", seen: false },
    { id: 3, text: "I'm working on the backend API. Almost done with the authentication system.", sender: "user", time: "10:33 AM", seen: true },
    { id: 4, text: "That's great! Let me know when you need me to integrate the frontend.", sender: "other", time: "10:35 AM", seen: false },
    { id: 5, text: "Sure, I'll probably finish by tomorrow. Then we can test everything together.", sender: "user", time: "10:36 AM", seen: false },
  ];
      ...
      <div className="w-full max-w-4xl bg-[#1A1D23] rounded-xl shadow-2xl overflow-hidden border border-gray-800 flex flex-col h-[85vh]">
        ...
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar bg-gradient-to-b from-gray-900 to-[#1A1D23]">
          <div className="space-y-6">
            ...
            {messages.map((message) => (
              ...
              >
                <div className={`flex ${message.sender === "user" ? "flex-row-reverse" : "flex-row"} max-w-[75%] gap-3`}>
                  {/* User Avatar */}
                  <div className="flex-shrink-0">
                    <Image
                      ...
                      src={message.sender === "user" ? "/icons/logo.png" : "/icons/logo.png"}
                      alt="User"
                    />
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className={`flex items-center mt-1 ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
                      ...
                      {/* Seen indicator for user's messages only */}
                      {message.sender === "user" && (
                        <span className={`text-xs ${message.seen ? "text-blue-400" : "text-gray-500"}`}>
                          {message.seen ? "✓✓" : "✓"}
                        </span>
                      )}
                    </div>
        ...
        <div className="p-4 border-t border-gray-800 bg-[#1A1D23]">
          <div className="flex items-center space-x-3">
            ...
            {/* Send Button */}
            <button className="p-2 rounded-full bg-orange-500 hover:bg-orange-600 transition-colors">
